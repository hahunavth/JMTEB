...
class EchoParser(nn.Module):
    ...
    def _tokenize_piece(self, x, template_piece):
        if isinstance(template_piece, str):
            ...
            # NOTE: Verify that all template piece have been replaced 
            # to identify cases where the input x does not contain all required template elements.
            if "%%" in template_piece:
                raise Exception(f"Not found template:", template_piece)
...
class EchoEmbeddingsGemma2b(nn.Module):
    def __init__(self, model, parser=None, pooling=None):
        super(EchoEmbeddingsGemma2b, self).__init__()
        self.model = model
        self.parser = parser
        self.pooling = pooling
    ...
    def forward(self, xs):
        ...
        return self.pooling(xs)

    @staticmethod
    def from_pretrained(base_model_path, parser, pooling, **kwargs):
        base_model = AutoModelForCausalLM.from_pretrained(base_model_path, **kwargs)
        base_model = base_model.to(device)
        return EchoEmbeddingsGemma2b(base_model, parser=parser, pooling=pooling)